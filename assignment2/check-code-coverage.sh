...
xcrun llvm-profdata merge default.profraw -output=$PROFDATA
...
if hash llvm-cxxfilt 2>/dev/null; then
    xcrun llvm-cov report -show-functions=1 -Xdemangler=llvm-cxxfilt $EXE -instr-profile=$PROFDATA *.cpp
else
    xcrun llvm-cov report -show-functions=1 $EXE -instr-profile=$PROFDATA *.cpp
fi

  echo "====================================================="
  echo "The lines below were never executed"
  echo "====================================================="
xcrun llvm-cov show $EXE -instr-profile=$PROFDATA | grep " 0|"
